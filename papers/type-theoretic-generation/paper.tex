...
\begin{itemize}[nosep]
  ...
  \item $C(a)$ is a cost function encoding pragmatic preferences (brevity, relevance, style).
\end{itemize}
...
\begin{theorem}[Correct Abstention]\label{thm:abstention}
If type $A$ is uninhabited in context $\Gamma$, then the type-theoretic generation procedure returns $\bot$.
This is the \emph{correct} response: there is no valid claim of type $A$ given the available knowledge.
\end{theorem}

\begin{proof}
The proof search procedure (Section~\ref{sec:proof-search}) exhaustively explores all derivation trees up to the search depth $d$.
If $A$ is uninhabited, no derivation exists at any depth.
The procedure returns $\bot$ after exhausting the search space.
By the soundness theorem (\Cref{thm:soundness}), the only alternative to returning $\bot$ would be returning a term $a$ with a valid derivation---but no such term exists.
Thus $\bot$ is the unique correct response.
\end{proof}
...
If the diagram $D : \mathcal{I} \to \Sem$ is \emph{coherent} (all diagram morphisms compose consistently) and the weighted limit $\lim^W D$ exists, then the attention output is a semantically valid object in $\Sem$.
If the diagram is incoherent (some diagram squares do not commute), then $\lim^W D$ does not exist, and the system abstains.
\end{theorem}

\begin{proof}
When the diagram is coherent, all required compatibility conditions for the limit cone are satisfiable.
The limit object $L = \lim^W D$ is then an object of $\Sem$ by construction, and the cone maps $\pi_i : L \to D(i)^{W(i)}$ witness its compatibility with every context element.
This is a semantically valid object because it type-checks against every constraint imposed by the diagram.

When the diagram is incoherent, there exist $i \xrightarrow{f} j \xrightarrow{g} k$ and $i \xrightarrow{h} k$ in $\mathcal{I}$ with $D(g) \circ D(f) \neq D(h)$.
...
Hence no limit exists, and the system correctly abstains rather than producing an incoherent blend.
\end{proof}
...
We use GHC extensions \texttt{GADTs}, \texttt{DataKinds}, and \texttt{RankNTypes} to encode dependent types at the Haskell level.
...
\newblock \emph{Journal of Automated Reasoning}, 60(2):195--340, 2018.
