...
\begin{enumerate}[nosep]
  ...
  \item We prove four proof cases and provide a conjectural fifth, establishing a correspondence between hallucination types and homotopy-theoretic invariants (\cref{sec:five-cases}): circular reasoning ($\pi_1$), unjustified inference ($\pi_0$), inconsistent justifications ($\pi_2$), fabricated entity chains ($H_n$), and compositional drift (holonomy, conjectural).
  ...
  \item We provide an executable Haskell formalization with all five topological obstruction detectors (\cref{sec:haskell}).
\end{enumerate}
...
\begin{remark}[Categorical Conventions]
\label{rem:categorical-conventions}
Throughout this paper, $\Sem$ is defined as an $(\infty,1)$-category (\cref{def:sem}), in which morphisms represent directed entailment and are not generally invertible. However, the homotopy-theoretic arguments in \cref{sec:five-cases}---particularly those involving $\pi_n$ and holonomy---require passage to the \emph{core groupoid} $\Sem^{\mathrm{gpd}}$, the maximal sub-$\infty$-groupoid obtained by restricting to invertible morphisms (equivalences). This is standard practice: any $(\infty,1)$-category $\mathcal{C}$ has a core $\mathcal{C}^{\simeq}$ that is an $\infty$-groupoid, and the homotopy groups $\pi_n(\mathcal{C})$ are defined as $\pi_n(\mathcal{C}^{\simeq})$. When we write $\pi_n(\Sem)$, we mean $\pi_n(\Sem^{\mathrm{gpd}})$. The full $(\infty,1)$-category structure of $\Sem$ is relevant for directed inference (entailment that is not reversible), while the core groupoid captures the equivalence structure needed for homotopy invariant computations. This distinction is important: semantic entailment $A \vdash B$ need not be invertible ($B \vdash A$ may fail), so $\Sem$ is genuinely an $(\infty,1)$-category, not an $\infty$-groupoid. The five proof cases in \cref{sec:five-cases} detect obstructions in $\Sem^{\mathrm{gpd}}$ that are invisible to vector space representations.
\end{remark}
...
\begin{proposition}[Homotopical Triviality of $\Vect$ for Semantic Purposes]
\label{prop:contractible}
The category $\Vect_{\R}^{\mathrm{fin}}$, viewed as an $(\infty,1)$-category, has contractible mapping spaces: for any two objects $V, W$, the space $\Hom_{\Vect}(V, W)$ is either empty or contractible. In particular, the homotopy invariants $\pi_k$ for $k \geq 1$ that are relevant to the semantic embedding (those of mapping spaces between fixed objects) are trivial.
\end{proposition}

\begin{proof}
For fixed finite-dimensional vector spaces $V$ and $W$, the space of linear maps $\Hom(V, W) \cong \R^{\dim(V) \cdot \dim(W)}$ is a vector space, hence contractible (it deformation-retracts to the origin via $t \mapsto t \cdot f$ for $t \in [0,1]$). In particular, $\pi_k(\Hom(V, W)) = 0$ for all $k \geq 0$.

We note that the \emph{core groupoid} of $\Vect_{\R}^{\mathrm{fin}}$ is \emph{not} contractible: its classifying space is $\coprod_{n \geq 0} B\mathrm{GL}_n(\R)$, which carries non-trivial topology (e.g., $\pi_1(B\mathrm{GL}_n(\R)) \cong \Z/2$ for $n \geq 1$ via the determinant, and $B\mathrm{GL}_n(\R)$ supports Stiefel--Whitney classes). However, what matters for the LLM embedding is that the functor $F: \Sem \to \Vect_{\R}^{\mathrm{fin}}$ maps semantic morphisms to \emph{linear maps}, and the space of linear maps between any two fixed vector spaces is contractible. Thus any two parallel semantic morphisms $f, g: A \to B$ with $F(f), F(g) \in \Hom(F(A), F(B))$ are connected by a canonical path in the codomain, destroying the distinction between them. The induced map $F_*: \pi_k(\Sem^{\mathrm{gpd}}) \to \pi_k(\Vect_{\R}^{\mathrm{fin}})$ sends all higher homotopy information of semantically relevant mapping spaces to zero.
\end{proof}
...
\begin{conjecture}[Compositional Drift as Non-Trivial Holonomy]
\label{conj:case5}
Compositional drift corresponds to non-trivial holonomy on the semantic fibration: transport along a loop in context space $B$ yields a non-identity automorphism of the fiber.
\end{conjecture}
...
\begin{quote}
\textbf{Heuristic argument.} Consider the semantic fibration $p: E \to B$ (\cref{def:fibration}). Suppose a connection $\nabla$ exists providing parallel transport. Let $a : A(\gamma)$ be a claim in the fiber over context $\gamma$, and let $\ell$ be a loop in context space $B$ based at $\gamma$.

The connection $\nabla$ would allow us to transport the claim $a$ along the loop $\ell$:
\[
...
The holonomy group at $\gamma$ would be:
\[
...
\begin{remark}[On the Formalization of Case~5]
\label{rem:case5-formalization}
A full formalization of Case~5 would require: (i)~constructing an explicit Ehresmann connection on the semantic fibration, where horizontal lifts correspond to context-preserving meaning shifts; (ii)~verifying that the induced parallel transport is well-defined (path-independent within each horizontal leaf); and (iii)~computing the holonomy group. The analogous construction for fiber bundles in differential geometry is classical (cf.\ Kobayashi--Nomizu), but its adaptation to the $(\infty,1)$-categorical setting of $\Sem$ requires further foundational work. The empirical evidence for compositional drift in LLMs is robust (see \cref{ex:drift}), and we expect that a formal connection can be constructed using the language of $\infty$-topoi~\cite{lurie2009}.
\end{remark}
...
\begin{remark}[Code Correspondence]
\label{rem:code-correspondence}
The companion Haskell module \texttt{TopologicalDetection.hs} implements all five detectors and the type-checking judgment. Due to Haskell's lack of full dependent types, the implementation uses \texttt{String} identifiers where the idealized version uses \texttt{SemTerm} endpoints (e.g., \texttt{PathType SemType String String} rather than \texttt{PathType SemType SemTerm SemTerm}). The GADT and DataKinds extensions are declared for the algebraic data type definitions but do not enforce the full type-theoretic invariants at compile time. The semantic content and detection logic are faithful to the paper; only the static type-level guarantees are weaker.
\end{remark}
...
\begin{lstlisting}[caption={Topological obstruction types (companion code uses String).}]
...
-- Check if a 1-chain is a cycle (in ker partial_1)
...
\begin{enumerate}[nosep]
  ...
  \item We \emph{conjecture} that the classification is complete: the five cases of \cref{thm:hhc} cover the homotopy-theoretic invariants that can obstruct faithful representation (see \cref{conj:completeness}).
\end{enumerate}

\begin{conjecture}[Completeness of the HHC]
\label{conj:completeness}
The five cases of \cref{thm:hhc}---$\pi_0$, $\pi_1$, $\pi_2$, $H_n$, and holonomy---exhaust all hallucination types that arise from topological obstructions.
\end{conjecture}

\begin{remark}[Evidence for Completeness]
\label{rem:completeness-evidence}
The evidence for \cref{conj:completeness} rests on the Postnikov tower of the semantic $\infty$-groupoid $\Sem^{\mathrm{gpd}}$. Recall that any space $X$ admits a Postnikov tower:
\[
\cdots \to X_{\langle 2 \rangle} \to X_{\langle 1 \rangle} \to X_{\langle 0 \rangle}
\]
where $X_{\langle n \rangle}$ is the $n$-truncation with $\pi_k(X_{\langle n \rangle}) = 0$ for $k > n$. The homotopy type of $X$ is determined by the homotopy groups $\{\pi_n(X)\}_{n \geq 0}$ together with the $k$-invariants (Postnikov invariants) $k_n \in H^{n+2}(X_{\langle n \rangle}; \pi_{n+1}(X))$. Our five cases cover: $\pi_0$ (Case~2), $\pi_1$ (Case~1), $\pi_2$ (Case~3), the integral homology groups $H_n$ (Case~4, which subsumes higher homotopy groups via the Hurewicz theorem in the simply-connected case), and the fibration structure (Case~5, capturing coherence failures under transport). Higher homotopy groups $\pi_k$ for $k \geq 3$ correspond to higher-order coherence failures in justification chains; these are detected by the $H_n$ machinery of Case~4 via the Hurewicz homomorphism $h: \pi_n \to H_n$ (which is an isomorphism when $\pi_k = 0$ for $k < n$). A formal proof of completeness would require showing that the $k$-invariants introduce no hallucination types beyond those captured by the five cases. We leave this to future work.
\end{remark}
